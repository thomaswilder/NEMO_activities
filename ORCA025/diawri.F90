   ...
   SUBROUTINE dia_wri( kt )
      ...
      IF ( iom_use("relvor") .OR. iom_use("absvor") .OR. iom_use("potvor") ) THEN
         ...
         DO jk = 1, jpkm1
            DO jj = 1, jpjm1
               DO ji = 1, fs_jpim1   ! vector opt.
                  zztmp  = (  e3t_n(ji,jj+1,jk)*tmask(ji,jj+1,jk) + e3t_n(ji+1,jj+1,jk)*tmask(ji+1,jj+1,jk)   &
                     &    + e3t_n(ji,jj  ,jk)*tmask(ji,jj  ,jk) + e3t_n(ji+1,jj  ,jk)*tmask(ji+1,jj  ,jk)  )
                  IF( zztmp /= 0._wp ) THEN   ;   zztmp = 4._wp / zztmp
                  ELSE                      ;   zztmp = 0._wp
                  ENDIF
                  z3d(ji,jj,jk) = zztmp * z3d(ji,jj,jk) 
               END DO
            END DO
         END DO
         CALL lbc_lnk( 'diawri', z3d, 'F', 1. )
      ...
      IF( (.NOT.l_ldfeiv_time) .AND. ( iom_use('RossRad')  .OR. iom_use('RossRadlim') &
         ...
         zztmp = 75._wp
         CALL ldf_eiv(kt, zztmp, z2d(:,:), z3d(:,:,1))
